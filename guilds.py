...
def guild(bot):
    ...
    async def join_guild(message):
        if message.from_user.is_bot==True or message.from_user.first_name=="Telegram":
            pass
        else:
            name=""
        ...
        if message.from_user.is_bot==True or message.from_user.first_name=="Telegram":
            ...
            match message.reply_to_message:
                case None:
                    ...
                    user1=search_info_user(message.from_user.id,message.chat.id)[0]
                ...
                case _:
                    ...
                    user=search_info_user(message.from_user.id,message.chat.id)[0]
                    guild_id=check_user_guild(message.from_user.id,message.chat.id)
                    owner=search_owner(guild_id)
                    if owner==user:
                        if member==None:
        ...
        if message.from_user.is_bot==True or message.from_user.first_name=="Telegram":
            ...
            match message.reply_to_message:
                case None:
                    ...
                    if owner==user1:
                        if member1==member:
                            change_rights(search_info_user(user,message.chat.id)[0],member)
                ...
                case _:
                    ...
                    if owner==user:
                        if member1==member:
                            change_rights(search_info_user(message.reply_to_message.from_user.id,message.chat.id)[0],member)
                            await bot.send_message(message.chat.id,"Вы передали права на гильдию",reply_to_message_id=message.id)
                        else:
                            await bot.send_message(message.chat.id,"Вы не можете передать права на гильдию члену другой гильдии",reply_to_message_id=message.id)
                    else:
    ...
    @bot.message_handler(regexp='Удалить гильдию|Delete Guild',chat_types=["supergroup","group"])
    async def del_guild(message):
        if message.from_user.is_bot==True or message.from_user.first_name=="Telegram":
            pass
        else:
            guild_id=check_user_guild(message.from_user.id,message.chat.id)
            match guild_id:
                case None:
                    await bot.send_message(message.chat.id,"Вы не можете удалить гильдию не находясь в ней",reply_to_message_id=message.id)
                case _:
                    owner_id=search_owner(guild_id)
                    user_id=search_info_user(message.from_user.id,message.chat.id)[0]
                    if owner_id==user_id:
                        markup_confirm=types.InlineKeyboardMarkup()
                        btn_confirm1=types.InlineKeyboardButton(text="Да",callback_data="Подтверждение_Да")
                        btn_confirm2=types.InlineKeyboardButton(text="Нет",callback_data="Подтверждение_Нет")
                        markup_confirm.add(btn_confirm1,btn_confirm2)
                        mes=await bot.send_message(message.chat.id,"Вы уверены?\nПри удалении вещи, находящиеся в гильдии, будут <b><u>утеряны</u></b>",reply_markup=markup_confirm,parse_mode="HTML",reply_to_message_id=message.id)
                        @bot.callback_query_handler(lambda call: call.data.startswith('Подтверждение_'))
                        async def handle_callback(call):
                            match call.data:
                                case "Подтверждение_Да":
                                    if call.from_user.id==message.from_user.id:
                                        delete_guild(guild_id)
                                        await bot.edit_message_text("Гильдия удалена",message.chat.id,mes.id)
                                    else:
                                        pass
                                case "Подтверждение_Нет":
                                    if call.from_user.id==message.from_user.id:
                                        await bot.delete_messages(message.chat.id,[mes.id,message.id])
                                    else:
                                        pass
                    else:
                        await bot.send_message(message.chat.id,"Вы не можете удалить гильдию не являясь её владельцем",reply_to_message_id=message.id)